...
import { StoreData, createStore, getStore, updateStore } from '../../../api/stores';
...
const Form: FC<Props> = ({ mode }) => {
	...
	const process = async (payload: Inputs) => {
		if (mode === 'add') {
			return await createStore(routes.ADMINISTRATOR, payload);
		}

		return await updateStore(routes.ADMINISTRATOR, params.id, payload);
	};
	...
	const fetch = async () => {
		setProcessing(true);
		try {
			const store = await getStore(routes.ADMINISTRATOR, params.id);
		...
		<IonPage>
			<IonContent fullscreen>
				<form onSubmit={handleSubmit(submit)}>
					<IonCard>
						<IonCardHeader>
							<IonCardTitle>{upperFirst(mode)} Store</IonCardTitle>
						</IonCardHeader>
					</IonCard>
					<div className='flex'>
